...
def is_affirmative(text: str) -> bool:
    ...
    "차 좀 불러줘"와 같은 동사인데도 확인 답변으로는 인식 못 함). 같은 날 후속
    통화에서 "어 진행해 줘"도 두 번이나 인식 못 해 반복된 것을 확인해 "진행"도
    같은 방식으로 추가했다. 이런 단어들은 문장 앞머리가 아니라 필러("어" 등) 뒤에
    오는 경우까지 잡아야 해서, 문장 앞머리가 아니라 어디에 있든 긍정으로 본다
    ("불러"/"진행"은 이 도메인에서 차량 호출을 진행하라는 의미로만 쓰여 다른 뜻과
    헷갈릴 위험이 낮다).
    """
    ...
    # "안 불러도 돼"/"못 불러"·"진행 안 해도 돼"처럼 부정어와 결합되면 그 단어가
    # 들어 있어도 긍정이 아니다 — is_negative()가 이런 문장을 따로 잡아 준다.
    calls_it = "불러" in compact and not has_any(compact, ["안불러", "못불러"])
    proceeds = "진행" in compact and not has_any(compact, ["안진행", "못진행", "진행안", "진행하지"])
    return (
