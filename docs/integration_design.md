...
| 이유 | 설명 |
|---|---|
| 의존성 | 분석기 쪽은 torch·google-genai·faster-whisper가 필요하고 TTS가 macOS 전용입니다 |
| 버전 | 구버전 `v4.py`는 `search_mode` 등을 아예 내보내지 않습니다. 키가 없어도 동작해야 합니다 |
...
- 배경: `docs/midm_vs_gemini_comparison.md`에 Mi:dm이 답변 끝에 한자를 흘린 사례가
  기록되어 있어, 문장 검사를 넣어 두었습니다.
